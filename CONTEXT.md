...
- A Connection may have concurrent Sessions inside one Remote main window. Each Session owns an independent protocol runtime and appears as a tab; switching tabs changes only the visible Session and does not disconnect the others.
- Double-clicking a Connection always opens and selects a new Session tab, including when that Connection already has an open Session. The explicit Connect action may instead offer the Operator a choice between the existing Session and another Session.
